...
from data.sr_select_dataset import select_dataset
from models.MMIR_SR_model_s2_c2_r2 import Model
...
def main(json_path: str = 'options/MMIR_SR_train_s2_c2_r2.json'):
    ...
    train_set: DatasetSuperResolution = select_dataset(opt_data_train, "train")
    train_loader: DataLoader[DatasetSuperResolution] = DataLoader(
        train_set,
    ...
    test_sets: List[DatasetSuperResolution] = select_dataset(opt_data_test, "test")
    test_loaders: List[DataLoader[DatasetSuperResolution]] = []
    ...
    for epoch in range(1, 1000000):  # keep running
        for train_data in tqdm(train_loader):
            ...
            if current_step % opt['train']['checkpoint_log'] == 0:
                GL, XL, XLL, YL, XH = model.log_train(current_step, epoch, logger, GL, XL, XLL, YL, XH,'debug/MMIR_SR_s2_c2_r2.png')
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                for test_loader in tqdm(test_loaders):
                    test_set: DatasetSuperResolution = test_loader.dataset
                    avg_psnr = 0.
                    avg_ssim = 0.
                ...
                start = time.time()
                logger.info(f"best model:{best_model},best PSNR:{best_psnr}")
                model.save(logger)
