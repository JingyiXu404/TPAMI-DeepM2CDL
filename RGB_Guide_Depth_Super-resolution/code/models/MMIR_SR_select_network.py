...
from models.MMIR_network import DCDicL as DCDicL_sr


def select_network(opt: Dict[str, Any]):
    opt_net = opt['netG']

    if opt_net['type'] == 'super_resolution':
        net = DCDicL_sr
        netG = net(n_iter_x=opt_net['x_iter'],
                   n_iter_y=opt_net['y_iter'],
                   rnn_iter=opt_net['rnn_iter'],
                   c_iter=opt_net['c_iter'],
                   in_nc=opt_net['in_nc'],
                   nc_x=opt_net['nc_x'],
