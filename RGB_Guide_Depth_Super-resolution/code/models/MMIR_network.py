...
class FB(nn.Module):
    def __init__(self, nc_x: List[int] = [64, 128, 256, 512], nc_d: List[int] = [16], out_nc: int = 1):
        ...
        self.conv_feature_x = nn.Sequential(nn.Conv2d(nc_x[0] * 2, nc_x[0], 3, padding=1, bias=False),
                                          nn.ReLU(inplace=True),
                                          nn.Conv2d(nc_x[0], nc_x[0], 3, padding=1, bias=False))
        self.conv_feature_y = nn.Sequential(nn.Conv2d(nc_x[0] * 2, nc_x[0], 3, padding=1, bias=False),
                                            nn.ReLU(inplace=True),
                                            nn.Conv2d(nc_x[0], nc_x[0], 3, padding=1, bias=False))
        self.conv_dictionary_x = nn.Sequential(nn.Conv2d(out_nc * nc_d[0], out_nc * nc_d[0], 3, padding=1, bias=False),
                                             nn.ReLU(inplace=True),
                                             nn.Conv2d(out_nc * nc_d[0], out_nc * nc_d[0], 3, padding=1, bias=False))
        self.conv_dictionary_y = nn.Sequential(nn.Conv2d(out_nc * nc_d[0], out_nc * nc_d[0], 3, padding=1, bias=False),
                                               nn.ReLU(inplace=True),
                                               nn.Conv2d(out_nc * nc_d[0], out_nc * nc_d[0], 3, padding=1, bias=False))
    ...
    def forward(self, u, v, dx, dy):
        ...
        '''
        a = torch.cat([u, v], dim=1)
        f = self.conv_feature_x(a)
        g = self.conv_feature_y(a)
        b = self.conv_dictionary_x(dx)
        e = self.conv_dictionary_y(dy)
        return f, g, b, e
...
class DCDicL(nn.Module):
    ...
    def forward(self, x,y, sigma,sigmay):
        ...
        for i in range(self.rnn_iter):
            ...
            # print('ax_in', i, ax[0][0, 0, :5, 0], dx[0][0, 0, 0, :5, 0])
            # print('ay_in', i, ay[0][0, 0, :5, 0], dy[0][0, 0, 0, :5, 0])
            ax, dx = self.body_listx[i](x, X, ax, dx,sigma)
            ay, dy = self.body_listy[i](y, Y, ay, dy,sigma) # get new items from the beginning for Y
            # print('ax_out', i, ax[0][0, 0, :5, 0], dx[0][0, 0, 0, :5, 0])
            # print('ay_out', i, ay[0][0, 0, :5, 0], dy[0][0, 0, 0, :5, 0])
            AX.append(ax)
            AY.append(ay)
            DX.append(dx)
            DY.append(dy)

            # update fax from ax and ay
            f, b = self.RB_list[i](ax, ay, dx)
            FA.append(f)
            #update ax and dx
            dx = b
            ax = f
            # print('fx_out', i, ax[0][0, 0, :5, 0], dx[0][0, 0, 0, :5, 0])
            # loss with z
